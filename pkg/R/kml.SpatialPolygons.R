kml.SpatialPolygonsDataFrame <- function(
...
#     stop('plotKML <3 "+proj=longlat +datum=WGS84"')
  ...
  if (is.na(colour) | !(colour %in% names(obj)))
    polygons.names <- 1:nrow(obj)
  else
    polygons.names <- obj[[colour]]
  ...
  for (i.poly in 1:nrow(obj)) {  # for each line
    ...
    if (!is.na(colour)) {
      write(paste('  <Style id="', 'poly', i.poly,'">', sep = ""), kml, append = TRUE)
      write("          <PolyStyle>", kml, append = TRUE)
      write(paste('  <color>', colour.values[i.poly], '</color>', sep = ""), kml, append = TRUE)
      write("		</PolyStyle>", kml, append = TRUE)
...
#     # write(" <Folder>", kml, append = TRUE)
    ...
    write(paste("\t\t<name>", current.poly.id, "</name>", sep = ""), kml, append = TRUE)
    write(paste('   <styleUrl>#poly', i.poly, '</styleUrl>', sep = ""), kml, append = TRUE)
    write("\t\t<Polygon>", kml, append = TRUE)
...
#     write(paste('		<tessellate>', as.numeric(tessellate), '</tessellate>', sep = ""), kml, append = TRUE)
    ...
    write("\t\t<outerBoundaryIs>", kml, append = TRUE)
    write("\t\t<LinearRing>", kml, append = TRUE)
    write("\t\t\t<coordinates>", kml, append = TRUE) 

    # For each vertice in the current line
    for (i.point in 1:current.poly.length) {
      write(paste('\t\t\t\t', current.poly.coords[i.point, 1], current.poly.coords[i.point, 2], elevation[i.point], sep = ", "), kml, append = TRUE)
    }
...
setMethod("kml", "SpatialPolygonsDataFrame", kml.SpatialPolygonsDataFrame) 
