  ...
  if(!is.na(proj4string(SGDF))){  #2
    if(!check_projection(SGDF)){ #3
      ...
      if(is.factor(SGDF@data[,var.name])){
        ...
        if(!missing(error.name)) { # if there is no error map then we assume that the map is perfect!
          if(missing(global.var) & is.numeric(SGDF@data[,var.name])) {
            global.var <- var(SGDF@data[,var.name], na.rm=TRUE)
          }
      ...
      warning('no reprojection required')
    }
  }
  else {
    stop("proj4 string required")
  }
}
