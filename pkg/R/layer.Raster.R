...
kml_layer.Raster <- function(
  ...
  if(!is.na(charmatch("z.lim", names(call)))){ 
    ...
    raster::image(obj, col = colour_scale, zlim = z.lim, frame.plot = FALSE, main="", maxpixels=ncell(r)) 
  } else {
    raster::image(obj, col = colour_scale, frame.plot = FALSE, main="", maxpixels=ncell(r))
  }
  dev.off()
