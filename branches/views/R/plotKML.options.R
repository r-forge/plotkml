...
## looks for FWTools in the Windows Registry Hive, the Program Files directory and the usr/bin installation

### Try to locate FWTools under various platforms:
paths <- function(gdalwarp = "", gdal_translate = "", convert = "", saga_cmd = "", show.paths = FALSE){
  ...
  if(.Platform$OS.type == "windows") {
    ...
    lt <- data.frame(gdalwarp, gdal_translate, convert, saga_cmd, stringsAsFactors = FALSE)
